...
import getopt
import sys
from search.dijsktra import dijsktraSearch
...
def main():
    ...
    for i in range(0, len(start_states)):    
        ...
        myMap = Map('dao-map/brc000d.map')
        diskstraRes = dijsktraSearch(start, goal, myMap)
        # myMap.plot_map(diskstraRes[2], start, goal, 'brc000dDijsktra')
        cost, expanded_diskstra = diskstraRes[0], diskstraRes[1] # replace None, None with the call to your Dijkstra's implementation
        time_end = time.time()
        ...
        time_start = time.time()
        myMap = Map('dao-map/brc000d.map')
        aStarRes = aStarSearch(start, goal, myMap)
        myMap.plot_map(aStarRes[2], start, goal, 'brc000dDijsktra')
        cost, expanded_astar = aStarRes[0], aStarRes[1] # replace None, None with the call to your A* implementation
        time_end = time.time()
